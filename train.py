...
writer = SummaryWriter('log_loss_all')
...
flowComp = model.UNet(6, 4)#光流输入，两个图片，RGB 6个通道，输出4个通道，x,y方向
#flowComp = model.UNet(6, 4)#光流输入，两个图片，RGB 6个通道，输出4个通道，x,y方向
flowComp.to(device)
ArbTimeFlowIntrp = model.UNet(20, 5)#第二次输入，两幅图像（6），两幅光流（4），两个扭曲图像g(10)，输出两个光流差（4）+一个遮挡判断（1）
...
trainFlowBackWarp      = model.backWarp(352, 352, device)#（反向传播，更新权重）
...
mean = [0.429, 0.431, 0.397]#是 R G B 三个通道上的均值
std  = [1, 1, 1]#三个通道的标准差
normalize = transforms.Normalize(mean=mean,
                                 std=std)#归一化
transform = transforms.Compose([transforms.ToTensor(), normalize])#将多种变换组合在一起
...
L1_lossFn = nn.L1Loss()
MSE_LossFn = nn.MSELoss()#l2loss
Smooth_loss = nn.SmoothL1Loss()

params = list(ArbTimeFlowIntrp.parameters()) + list(flowComp.parameters())
#构建一个优化器optimizer，你必须给它一个可进行迭代优化的包含了所有参数,可以指定程序优化特定的选项，例如学习速率，权重衰减
optimizer = optim.Adam(params, lr=args.init_learning_rate)
# scheduler to decrease learning rate by a factor of 10 at milestones.
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.milestones, gamma=0.1)#按设定的间隔调整学习率


###Initializing VGG16 model for perceptual loss


vgg16 = torchvision.models.vgg16(pretrained=True)#pretrained=True是指使用预训练的权重
...
def validate():
    ...
    with torch.no_grad():
        for validationIndex, (validationData, validationFrameIndex) in enumerate(validationloader, 0):
            ...
            #取三帧，前两帧送入flowComp得到双向光流F_0_1，F_1_0
            
            flowOut = flowComp(torch.cat((I0, I1), dim=1))
            F_0_1 = flowOut[:,:2,:,:]
            F_1_0 = flowOut[:,2:,:,:]

            #得中间图像g_I0_F_t_0，g_I1_F_t_1
            ...
            #输出残差光流和遮挡处理 ΔF_t_0_f(2) + ΔF_t_1_f(2) + V_t_0(1) = 5
            F_t_0_f = intrpOut[:, :2, :, :] + F_t_0
            F_t_1_f = intrpOut[:, 2:4, :, :] + F_t_1
            V_t_0   = torch.sigmoid(intrpOut[:, 4:5, :, :])
            V_t_1   = 1 - V_t_0#映射关系的约束
            ...
            prcpLoss = MSE_LossFn(vgg16_conv_4_3(Ft_p), vgg16_conv_4_3(IFrame))#提取特征并计算语义差
            #扭曲损失
            warpLoss = Smooth_loss(g_I0_F_t_0, IFrame) + Smooth_loss(g_I1_F_t_1, IFrame) + Smooth_loss(validationFlowBackWarp(I0, F_1_0), I1) + Smooth_loss(validationFlowBackWarp(I1, F_0_1), I0)
            ...
            #平滑损失
            loss = 204 * recnLoss + 102 * warpLoss + 0.005 * prcpLoss + loss_smooth#总损失
            tloss += loss.item()
            
            #psnr
            MSE_val = MSE_LossFn(Ft_p, IFrame)#求MSE
            psnr += (10 * log10(1 / MSE_val.item()))#求psnr
...
for epoch in range(dict1['epoch'] + 1, args.epochs):
    ...
    for trainIndex, (trainData, trainFrameIndex) in enumerate(trainloader, 0):
        ...
        # Calculate flow between reference frames I0 and I1:计算参考系 I0 和 I1 之间的流量
        flowOut = flowComp(torch.cat((I0, I1), dim=1))
        ...
        # Calculate intermediate flows:合成中间近似光流
        F_t_0 = fCoeff[0] * F_0_1 + fCoeff[1] * F_1_0
        F_t_1 = fCoeff[2] * F_0_1 + fCoeff[3] * F_1_0
